...
filename = "print_to_this_file.log"
train = 1
output = "softmax"
#X_train
#X_test
...
file, stdout_original = utils.setPrintToFile("{0}.txt".format(filepath))
...
if word_embeddings:
    word_input = keras.layers.Input((maxWords,))
    inputs.append(word_input)

    if pretrained_embedding == "":
        word_embedding = keras.layers.Embedding(nbr_words, word_embedding_size)(word_input)
    else:
        # Retrieve embeddings
        embedding_matrix = utils.word2VecEmbeddings(word2ind, word_embedding_size)
        word_embedding = keras.layers.Embedding(nbr_words, word_embedding_size, weights=[embedding_matrix], trainable=pretrained_embedding, mask_zero=False)(word_input)
    embeddings_list.append(word_embedding)

# Input - Characters Embeddings

if maxChar != 0:
    character_input = keras.layers.Input((maxWords,maxChar,))
...
embeddings = keras.layers.concatenate(embeddings_list) if len(embeddings_list)==2 else embeddings_list[0]
model = keras.layers.Dropout(dropout)(embeddings)
model = keras.layers.Bidirectional(keras.layers.LSTM(lstm_hidden, return_sequences=True, dropout=dropout))(model)
model = keras.layers.Dropout(dropout)(model)


if output == "crf":
    # Output - CRF
    crfs = [[keras_contrib.layers.CRF(out_size),out_size] for out_size in [len(x)+1 for x in ind2label]]
    outputs = [x[0](keras.layers.Dense(x[1])(model)) for x in crfs]
    model_loss = [x[0].loss_function for x in crfs]
    model_metrics = [x[0].viterbi_acc for x in crfs]

if output == "softmax":
        outputs = [keras.layers.Dense(out_size, activation='softmax')(model) for out_size in [len(x)+1 for x in ind2label]]
...
classification_scores = utils.Classification_Scores(
    [X_train, y_train],
    ind2label,
...
if train:
    ...
    utils.save_model_training_scores("{0}".format(filepath), hist, classification_scores)
    ...
    # Best epoch results
    best_results = utils.model_best_scores(classification_scores, best_epoch)

# Load weigths from best training epoch into model
keras_contrib.utils.save_load_utils.load_all_weights(model, best_model_weights_path)
...
if gen_confusion_matrix:
    for i, y_target in enumerate(y_test):
        ...
        predictions, target = utils.compute_predictions(model, X_test, y_target, ind2label[i])
        # Generate confusion matrices
        utils.save_confusion_matrix(target, predictions,  list(ind2label[i].values()), "{0}_task_{1}_confusion_matrix_test".format(filepath,str(i+1)))
...
if validation:
    ...
    for i, y_target in enumerate(y_valid):
        # Compute predictions, flatten
        predictions, target = utils.compute_predictions(model, X_valid, y_target, ind2label[i], nbrTask=i)
        ...
        # Generate confusion matrices
        utils.save_confusion_matrix(target, predictions,  list(ind2label[i].values()), "{0}_task_{1}_confusion_matrix_validation".format(filepath,str(i+1)))


# Close file
utils.closePrintToFile(file, stdout_original)
print(end_string)
...
def character_embedding_layer(layer_type, character_input, maxChar, nbr_chars, char_embedding_size,
    ...
    embed_char_out = keras.layers.TimeDistributed(keras.layers.Embedding(nbr_chars, char_embedding_size), name='char_embedding')(character_input)
    embed_char = keras.layers.TimeDistributed(keras.layers.Flatten())(embed_char_out)

    if layer_type == "CNN":
        conv1d_out = keras.layers.TimeDistributed(keras.layers.Convolution1D(kernel_size=cnn_kernel_size, filters=cnn_filters, padding='same'))(embed_char)
        char_emb = keras.layers.TimeDistributed(keras.layers.MaxPooling1D(maxChar))(conv1d_out)

    if layer_type == "BILSTM":
        char_emb= keras.layers.Bidirectional(keras.layers.LSTM(lstm_units,return_sequences=True))(embed_char)
